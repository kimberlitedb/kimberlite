...
- **Minor bump (0.x → 0.y)** may introduce breaking changes. Every breaking change is flagged with a **`**BREAKING**:`** marker in `CHANGELOG.md` and called out in the release notes with migration guidance.
- **Patch bump (0.x.y → 0.x.z)** is guaranteed non-breaking. Bug fixes, documentation, internal refactors, new non-breaking APIs only.
- **Unreleased → 0.x.y** always goes through `CHANGELOG.md` with an explicit section.
...
- CHANGELOG entries that omit the `**BREAKING**:` marker on a minor bump fail the release check.
- The `just release-check` recipe runs both of the above locally.
