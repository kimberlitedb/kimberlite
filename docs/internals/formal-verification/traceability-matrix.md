...
**PR-gating promotion (2026-04, v0.5.0):** The five EPYC-proven
theorems above were promoted from nightly-only
(`formal-verification-aspirational.yml`) to PR-blocking
(`formal-verification.yml::tla-tlaps-pr`). They run at `--stretch 300`
to fit the GitHub-hosted runner's 20-minute wallclock budget per file
(Compliance_Proofs.tla and VSR_Proofs.tla together); full `--stretch
3000` and `--stretch 10000` runs continue nightly and on the EPYC box
respectively. If PR-stretch is too tight in practice (OOM or timeout),
the individual theorem is demoted back to aspirational-only here and
the PR job moves to a narrower `--verify-theorem` invocation — never
silently skipped.
...
| 3 | **ViewMonotonicity** — view numbers never decrease | `specs/tla/VSR.tla::ViewMonotonicity`; `specs/tla/VSR_Proofs.tla::ViewMonotonicityTheorem` (TLAPS-verified, 2026-04; **PR-gated at `--stretch 300` in 2026-04** per ROADMAP v0.5.0 "TLAPS PR-gating" item) | `crates/kimberlite-vsr/src/replica/normal.rs` (production `assert!` in Phase 4); `crates/kimberlite-vsr/src/kani_proofs.rs::verify_view_number_monotonic` | TLC, **TLAPS ✅**, Kani, production assert | **PR (TLAPS, stretch 300)** + aspirational (TLAPS stretch 3000) + EPYC |
...
| 14 | **TenantIsolation** — cross-tenant reads/writes are rejected | `specs/tla/Compliance.tla`; `specs/tla/Compliance_Proofs.tla::TenantIsolationTheorem` (TLAPS-verified, 2026-04; **PR-gated at `--stretch 300` in 2026-04**) | `crates/kimberlite-directory/src/`; `crates/kimberlite-server/src/lib.rs` (tenant_id check); `#[flux(...)]` refinements in `kimberlite-types` | TLC, **TLAPS ✅**, Coq, Flux | **PR (TLAPS, stretch 300)** + aspirational (stretch 3000) + EPYC |
...
| 18 | **EncryptionAtRest** — all data is encrypted when stored | `specs/tla/Compliance.tla::EncryptionAtRest`; `specs/tla/Compliance_Proofs.tla::EncryptionAtRestTheorem` (TLAPS-verified, 2026-04; **PR-gated at `--stretch 300` in 2026-04**) | `crates/kimberlite-storage/src/lib.rs` (encrypted=true invariant); `crates/kimberlite-crypto/src/verified/aes_gcm.rs` | TLC, **TLAPS ✅**, production assert | **PR (TLAPS, stretch 300)** + aspirational (stretch 3000) + EPYC |
| 19 | **AccessControlCorrect** — CanAccess ⇒ same tenant | `specs/tla/Compliance.tla::AccessControlCorrect`; `specs/tla/Compliance_Proofs.tla::AccessControlCorrectnessTheorem` (TLAPS-verified, 2026-04; **PR-gated at `--stretch 300` in 2026-04**) | `crates/kimberlite-rbac/src/` | TLC, **TLAPS ✅** | **PR (TLAPS, stretch 300)** + aspirational (stretch 3000) + EPYC |
...
| 29 | **SafetyProperties (VSR composite)** — conjunction of the eight VSR safety invariants | `specs/tla/VSR_Proofs.tla::SafetyPropertiesTheorem` (tool-heterogeneous composition, 2026-04; **file-level PR-gated at `--stretch 300` via VSR_Proofs.tla verify step in 2026-04**) | — | **TLAPS ✅** (cited) + underlying constituents covered by TLC/Ivy/Alloy | **PR (TLAPS, stretch 300)** + aspirational (stretch 3000) + EPYC |
