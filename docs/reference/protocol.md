...
**Note**: Optimistic concurrency control is implemented in the kernel but not yet exposed in the wire protocol. The kernel supports an `expected_offset` field in `AppendBatch` commands that validates the stream hasn't advanced before appending. This will be added to the wire protocol in a future version with error code 16 (`OffsetMismatch`). See [ROADMAP.md](../../ROADMAP.md) for details.
...
- Error code 16 (`OffsetMismatch`) is planned for optimistic concurrency control (see [ROADMAP.md](../../ROADMAP.md))
