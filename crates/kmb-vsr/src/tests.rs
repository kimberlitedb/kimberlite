...
use kmb_types::{DataClass, Placement};
...
fn log_entry_with_idempotency_id() {
    let id = kmb_types::IdempotencyId::generate();
...
fn single_node_effects_include_storage_append() {
    ...
    let stream_id = kmb_types::StreamId::new(0); // Auto-allocated ID
    let append_cmd = kmb_kernel::Command::append_batch(
        stream_id,
        vec![bytes::Bytes::from("event-1"), bytes::Bytes::from("event-2")],
        kmb_types::Offset::ZERO,
    );
    ...
    }) = storage_append
    {
        ...
        assert_eq!(*base_offset, kmb_types::Offset::ZERO);
    }
}
...
fn single_node_log_entries_are_sequential() {
    ...
    for i in 1..=10 {
        let cmd = kmb_kernel::Command::create_stream_with_auto_id(
...
fn single_node_replicator_trait_is_object_safe() {
    ...
    fn submit_via_trait(r: &mut dyn Replicator, cmd: kmb_kernel::Command) -> crate::OpNumber {
