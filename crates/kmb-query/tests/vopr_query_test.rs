...
use kmb_sim::query_workload::{QueryWorkloadGenerator, TableSchema};
...
fn test_query_determinism_checker() {
    ...
    assert!(!check3.is_ok(), "Different results should violate determinism");
...
fn test_read_your_writes_checker() {
    ...
    assert!(!check2.is_ok(), "Wrong value should violate read-your-writes");
...
fn test_order_by_limit_checker() {
    ...
    assert!(!check2.is_ok(), "Wrong rows should violate ORDER BY + LIMIT correctness");
...
fn test_aggregate_correctness_checker() {
    ...
    assert!(!check2.is_ok(), "Wrong count should violate aggregate correctness");
    ...
    assert!(!check4.is_ok(), "Wrong sum should violate aggregate correctness");
...
fn test_workload_generator() {
    ...
    let aggregate = generator.generate_aggregate().expect("Should generate aggregate");
    assert!(aggregate.starts_with("SELECT"));
    assert!(aggregate.contains("COUNT") || aggregate.contains("SUM") ||
            aggregate.contains("MIN") || aggregate.contains("MAX"));
    ...
    assert_eq!(queries_a, queries_b, "Same seed should produce same queries");
}
