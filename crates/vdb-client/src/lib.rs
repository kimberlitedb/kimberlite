//! # vdb-client: RPC client for `VerityDB`
//!
//! This crate provides a synchronous RPC client for communicating with
//! a `VerityDB` server using the binary wire protocol defined in `vdb-wire`.
//!
//! ## Usage
//!
//! ```ignore
//! use vdb_client::{Client, ClientConfig};
//! use vdb_types::{DataClass, TenantId};
//!
//! // Connect to server
...
//!
//! // Read events back
//! let events = client.read_events(stream_id, vdb_types::Offset::new(0), 1024)?;
//!
//! // Execute a query
...
pub use vdb_wire::{QueryParam, QueryResponse, QueryValue, ReadEventsResponse};
