//! `VerityDB` administration CLI.
//!
//! Provides commands for managing streams, querying data, and
//! administering a `VerityDB` instance.

use anyhow::{Context, Result};
use clap::{Parser, Subcommand};
use vdb_client::{Client, ClientConfig, QueryParam};
use vdb_types::{DataClass, Offset, StreamId, TenantId};

/// `VerityDB` administration CLI.
#[derive(Parser)]
#[command(name = "vdb-admin")]
#[command(about = "VerityDB administration CLI", long_about = None)]
struct Cli {
    /// Server address.
        ...
        Commands::Query { sql, at } => {
            ...
            for row in &result.rows {
                let values: Vec<String> = row
                    .iter()
                    .map(|v| match v {
                        vdb_wire::QueryValue::Null => "NULL".to_string(),
                        vdb_wire::QueryValue::BigInt(n) => n.to_string(),
                        vdb_wire::QueryValue::Text(s) => s.clone(),
                        vdb_wire::QueryValue::Boolean(b) => b.to_string(),
                        vdb_wire::QueryValue::Timestamp(t) => t.to_string(),
                    })
                    .collect();
                println!("{}", values.join("\t"));
            }
