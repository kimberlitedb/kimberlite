...
use bytes::Bytes;
use kmb_kernel::traits::{Clock, Storage};
use kmb_kernel::{apply_committed, Command, ReplicaId, Runtime, State};
use kmb_sim::kernel_adapter::{ClockAdapter, NetworkAdapter, StorageAdapter};
use kmb_sim::{NetworkConfig, SimClock, SimNetwork};
...
fn kernel_runs_under_simulation() {
    ...
    runtime.execute_effects(effects).expect("effects should execute");
