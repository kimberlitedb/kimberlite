        ...
        if let Some(cached_result) = self.query_cache.get(&key) {
            ...
            if cached_result != result {
                return InvariantResult::Violated {
                    ...
                    message: format!(
                        "query returned different results for same inputs: {}",
                        sql
                    ),
                    context: vec![
...
impl TypeSafetyChecker {
    ...
    fn types_compatible(&self, expected: &str, actual: &str) -> bool {
        ...
        let numeric_types = ["TINYINT", "SMALLINT", "INTEGER", "BIGINT", "REAL", "DECIMAL"];
        ...
        if let Some(full_result) = self.full_results.get(query_key) {
            ...
            if limited_rows != expected {
                return InvariantResult::Violated {
                    ...
                    message: format!(
                        "ORDER BY + LIMIT returned wrong rows: query={}",
                        query_key
                    ),
                    context: vec![
...
mod tests {
    ...
    fn query_determinism_checker_tracks_params() {
        ...
        checker.check_query("SELECT * FROM users WHERE id = $1", &["1".to_string()], "result1");
        checker.check_query("SELECT * FROM users WHERE id = $1", &["2".to_string()], "result2");
    ...
    fn all_checkers_implement_trait() {
        let mut determinism: Box<dyn InvariantChecker> =
            Box::new(QueryDeterminismChecker::new());
        ...
        let mut aggregate: Box<dyn InvariantChecker> =
            Box::new(AggregateCorrectnessChecker::new());
