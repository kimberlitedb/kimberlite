...
fn serde_roundtrip_with_regions() {
    ...
    assert_eq!(
        deserialized.group_for_placement(&Placement::Global).unwrap(),
        GroupId::new(0)
    );
    assert_eq!(
        deserialized
...
fn large_number_of_regions() {
    ...
    for i in 1..=100 {
        let result = directory.group_for_placement(&Placement::Region(Region::custom(format!(
            "region-{}",
            i
        ))));
...
fn directory_inequality() {
    ...
    assert_ne!(dir1, dir2, "Different group IDs should make directories unequal");

    let dir3 = Directory::new(GroupId::new(99)).with_region(Region::USEast1, GroupId::new(1));

    assert_ne!(dir1, dir3, "Different global groups should make directories unequal");
}
