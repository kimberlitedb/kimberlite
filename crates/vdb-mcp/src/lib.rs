//! `VerityDB` MCP Server
//!
//! This crate implements a Model Context Protocol (MCP) server for `VerityDB`,
...
//!
//! - **`verity_query`**: Execute SQL queries with automatic access control and transformation
//! - **`verity_export`**: Bulk data export with anonymization
//! - **`verity_verify`**: Verify integrity of previously exported data
//! - **`verity_list_tables`**: Discover available tables based on access scope
//!
//! # Security
//!
//! All tool invocations require a valid access token (from `vdb-sharing`). The token
//! determines:
//!
...
//! use std::sync::Arc;
//! use vdb::Verity;
//! use vdb_sharing::TokenStore;
//! use vdb_mcp::McpServer;
//!
//! // Create the MCP server
//! let db = Arc::new(Verity::open("./data")?);
