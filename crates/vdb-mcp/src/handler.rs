...
use vdb_query::Value;
use vdb_sharing::{ExportScope, TokenId, TokenStore, TransformationType};
...
impl ToolHandler {
    ...
    pub fn new(db: Arc<Verity>, tokens: Arc<RwLock<TokenStore>>, audit: Arc<AuditLog>) -> Self {
    ...
    pub fn execute_query(&self, input: &QueryInput) -> McpResult<QueryOutput> {
        ...
        // Build audit record
        let mut audit_builder = AuditRecord::builder(token_id, tenant_id, "verity_query");
    ...
    pub fn execute_export(&self, input: &ExportInput) -> McpResult<ExportOutput> {
        ...
        let mut audit_builder =
            AuditRecord::builder(token_id, tenant_id, "verity_export").tables(&input.tables);
    ...
    pub fn execute_verify(&self, input: &VerifyInput) -> McpResult<VerifyOutput> {
        ...
        // Build audit record
        let audit_builder = AuditRecord::builder(token_id, tenant_id, "verity_verify")
    ...
    pub fn execute_list_tables(&self, input: &ListTablesInput) -> McpResult<ListTablesOutput> {
        ...
        // Build audit record
        let audit_builder = AuditRecord::builder(token_id, tenant_id, "verity_list_tables");
    ...
    fn validate_token(&self, token_str: &str) -> McpResult<(TokenId, TenantId, ExportScope)> {
        ...
        let scope = store.use_token(token_id).map_err(|e| match e {
            vdb_sharing::SharingError::TokenExpired => McpError::TokenExpired,
            vdb_sharing::SharingError::TokenRevoked => McpError::TokenRevoked,
            vdb_sharing::SharingError::TokenNotFound => {
    ...
    let rows: Vec<Vec<serde_json::Value>> = result
        ...
        .map(|row| {
            row.iter()
                .enumerate()
                .map(|(i, value)| {
                    let column = result.columns.get(i).map_or("", vdb::ColumnName::as_str);
...
mod tests {
    ...
    fn setup() -> (Arc<Verity>, Arc<RwLock<TokenStore>>, Arc<AuditLog>) {
        let temp_dir = TempDir::new().unwrap();
        let db = Arc::new(Verity::open(temp_dir.path()).unwrap());
