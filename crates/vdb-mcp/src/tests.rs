...
fn test_mcp_initialize_flow() {
    ...
    assert!(response.contains("verity-mcp"));

    // List tools
    let list_request = r#"{"jsonrpc":"2.0","id":2,"method":"tools/list","params":{}}"#;
    let response = server.handle_request(list_request);
    assert!(response.contains("verity_query"));
    assert!(response.contains("verity_export"));
    assert!(response.contains("verity_verify"));
    assert!(response.contains("verity_list_tables"));
}
...
fn test_token_validation() {
    ...
    let request = format!(
        r#"{{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{{"name":"verity_list_tables","arguments":{{"token":"{}"}}}}}}"#,
        token_str
    );
...
fn test_query_safety_validation() {
    ...
    let request = format!(
        r#"{{"jsonrpc":"2.0","id":1,"method":"tools/call","params":{{"name":"verity_query","arguments":{{"sql":"DROP TABLE users","token":"{}"}}}}}}"#,
        token_str
    );
    let response = server.handle_request(&request);
...
fn test_audit_logging() {
    ...
    assert_eq!(records[0].tool_name, "verity_list_tables");
}
