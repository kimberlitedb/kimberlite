//! Agent protocol types for `VerityDB` cluster management.
//!
//! This crate defines the protocol used for communication between `VerityDB`
...
//! - **Platform operators**: The official `VerityDB` platform
...
/// Unique identifier for a `VerityDB` cluster.
...
pub struct MetricSample {
    /// Metric name (e.g., "vdb.writes.total").
    pub name: String,
    /// Metric value.
    pub value: f64,
...
mod tests {
    ...
    fn metrics_batch_roundtrip() {
        let msg = AgentMessage::MetricsBatch {
            ...
            metrics: vec![
                MetricSample {
                    name: "vdb.writes.total".to_string(),
                    value: 12345.0,
                    timestamp_ms: 1700000000000,
                    labels: vec![("tenant".to_string(), "acme".to_string())],
                },
                MetricSample {
                    name: "vdb.reads.total".to_string(),
                    value: 98765.0,
                    timestamp_ms: 1700000000000,
                    labels: vec![],
                },
