        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &size, |b, _| {
            b.iter_batched(
                || {
                    ...
                    // Write data first
                    storage
                        .append_batch(stream_id, vec![Bytes::from(data.clone())], Offset::ZERO, None, true)
                        .unwrap();
                ...
                b.iter_batched(
                    || {
                        ...
                        let events: Vec<Bytes> =
                            (0..batch_size).map(|_| Bytes::from(vec![0u8; 256])).collect();
        ...
        group.bench_with_input(BenchmarkId::from_parameter(count), &count, |b, &count| {
            b.iter_batched(
                || {
                    ...
                    // Write test data
                    let events: Vec<Bytes> = (0..count).map(|_| Bytes::from(vec![0u8; 256])).collect();
                    storage.append_batch(stream_id, events, Offset::ZERO, None, true).unwrap();
