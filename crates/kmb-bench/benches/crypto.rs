...
use kmb_crypto::{
    ChainHash, EncryptionKey, FieldKey, SigningKey, chain_hash, decrypt_field,
    encrypt_field, internal_hash,
};
use kmb_crypto::encryption::{Nonce, encrypt, decrypt};
...
fn bench_aes_gcm_encrypt(c: &mut Criterion) {
    ...
    for size in [64, 256, 1024, 4096, 16384, 65536] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &plaintext, |b, plaintext| {
            b.iter(|| {
                let ciphertext = encrypt(black_box(&key), black_box(&nonce), black_box(plaintext));
...
fn bench_aes_gcm_decrypt(c: &mut Criterion) {
    ...
    for size in [64, 256, 1024, 4096, 16384, 65536] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(size), &ciphertext, |b, ciphertext| {
            b.iter(|| {
                let plaintext = decrypt(black_box(&key), black_box(&nonce), black_box(ciphertext))
