...
impl LatencyTracker {
    ...
    pub fn print_summary(&self, operation: &str) {
        ...
        println!("  p50:   {:>10} ns ({:>8.2} μs)", self.p50(), self.p50() as f64 / 1000.0);
        println!("  p95:   {:>10} ns ({:>8.2} μs)", self.p95(), self.p95() as f64 / 1000.0);
        println!("  p99:   {:>10} ns ({:>8.2} μs)", self.p99(), self.p99() as f64 / 1000.0);
        println!("  p99.9: {:>10} ns ({:>8.2} μs)", self.p999(), self.p999() as f64 / 1000.0);
        println!("  max:   {:>10} ns ({:>8.2} μs)", self.max(), self.max() as f64 / 1000.0);
        println!("  mean:  {:>10.0} ns ({:>8.2} μs)", self.mean(), self.mean() / 1000.0);
    }
}
