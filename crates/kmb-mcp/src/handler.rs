...
use kmb_query::Value;
use kmb_sharing::{ExportScope, TokenId, TokenStore, TransformationType};
...
impl ToolHandler {
    ...
    fn validate_token(&self, token_str: &str) -> McpResult<(TokenId, TenantId, ExportScope)> {
        ...
        let scope = store.use_token(token_id).map_err(|e| match e {
            kmb_sharing::SharingError::TokenExpired => McpError::TokenExpired,
            kmb_sharing::SharingError::TokenRevoked => McpError::TokenRevoked,
            kmb_sharing::SharingError::TokenNotFound => {
